...
- **Docker** — opsional, tapi diperlukan untuk uji flashlight (`psm-validate`, `psm-optimize`). Bisa dipasang belakangan, tidak memblok instalasi
...
Terima default dengan mengetik "semua default oke" — atau sebutkan hanya nilai yang ingin diubah.
...
Interaktif: tanya nama module & author, bangkitkan kerangka cross-version, tawarkan fungsi pemantik, lalu validasi otomatis.
...
Tiga lapis: pindai statis (selalu), uji flashlight Docker (bila Docker ada), review adversarial e-commerce (selalu). Output: laporan JSON per versi di `_bmad-output/psm-validate/`.
...
Tanya apa saja soal PrestaShop cross-version, brainstorm fungsi e-commerce, atau minta arahan ke workflow yang tepat.
...
| psm-validate | `/psm-validate` | Validasi & audit kompatibilitas |
