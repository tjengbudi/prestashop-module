...
Dua bukti diperlukan, karena lolos validate saja bisa menyembunyikan perubahan yang ternyata lebih lambat atau mengubah perilaku. **Kompatibilitas:** panggil psm-validate atas module hasil terhadap ketiga versi (lihat `{project-root}/skills/psm-validate/SKILL.md`); status lolos per versi adalah vonis JSON-nya — baca apa adanya. **Performa:** ukur ulang dengan profiler yang sama, lalu bandingkan dengan blok baseline di `.psm-optimize-plan.md` (bukan dari ingatan) untuk membuktikan perbaikan nyata. Module teroptimasi hanya bila psm-validate hijau di ketiga versi dan metrik membaik tanpa regresi perilaku. Bila ada error tersisa atau metrik tak membaik, tulis temuan itu kembali ke `.psm-optimize-plan.md` dan rancang ulang dari artefak — jangan menyatakan selesai. Ringkas ke Budi: apa yang dioptimasi, sebelum/sesudah per metrik, dan status lolos per versi.
