...
4. **Augment katalog bila ada.** Bila `{project-root}/_bmad/psm/memory/ecommerce/function-catalog.md` ada, baca untuk fungsi tambahan di luar `references/ecommerce-function-catalog.md`. Bila belum, lanjut — katalog inti sudah di-embed.
   **Breaking change versi target.** Bila `{project-root}/_bmad/psm/memory/tech/breaking-changes-<major>.md` ada untuk major di `psm_target_versions`, baca sebelum merancang — ia memuat batas versi **minor** (mis. Hummingbird/Bootstrap 5 di 9.1) yang sebagian besar hanya terbukti saat runtime — static scan cuma menangkap jejak tekstualnya, dan hanya bila target menyebut minor-nya (`9.1`, bukan `9`).
