...
def test_e2e_config_override():
    """Key psm_e2e_* di config.yaml section psm menang atas default kanonik."""
    override = ("document_output_language: English\n"
                "psm:\n"
                "  name: PrestaShop Module Builder\n"
                "  psm_e2e_enabled: 'false'\n"
                "  psm_e2e_browsers: chromium\n")
    proj = make_project(override)
    res = run(proj)
    data = json.loads(res.stdout)
    check("e2e override: enabled=false menang", data["psm_e2e_enabled"] == "false", res.stdout)
